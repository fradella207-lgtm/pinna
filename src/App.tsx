...
export default function App() {
  ...
  const listStorageKey = user ? `spotter_lists_${user.uid}` : "spotter_lists_local";
  ...
  const [activeTransport, setActiveTransport] = useState<string>("tutti");
  ...
  const filteredPlaces = useMemo(() => {
    ...
    return baseList.filter((place) => {
      ...
      // 4b. Transport Mode / Mezzo di trasporto
      if (activeTransport !== "tutti") {
        const m = (place.mezzo_trasporto || "").toLowerCase();
        const target = activeTransport.toLowerCase();
        const match = m.includes(target) || 
          (target === "piedi" && (m.includes("pied") || m.includes("trek") || m.includes("cammin"))) ||
          (target === "auto" && (m.includes("auto") || m.includes("macchina"))) ||
          (target === "moto" && m.includes("moto")) ||
          (target === "bici" && (m.includes("bici") || m.includes("mtb") || m.includes("ciclo"))) ||
          (target === "camper" && (m.includes("camper") || m.includes("van"))) ||
          (target === "treno_bus" && (m.includes("treno") || m.includes("bus") || m.includes("pubblic")));
        if (!match) return false;
      }
  ...
  }, [places, activeActivity, specialFilter, activeCountry, activeRegion, activeProvince, activeTransport, activeListId, searchQuery, experienceRecommendedPlaces]);
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden bg-slate-950 font-sans select-none">
      ...
      <div className="absolute top-4 left-3 right-3 sm:left-4 sm:right-4 max-w-3xl mx-auto z-20 pointer-events-auto flex items-center gap-2">
        <div className="relative flex-1 flex items-center gap-2 p-1.5 bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border border-slate-200/90 dark:border-slate-800 rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
          <div className="relative flex-1 flex items-center">
            ...
            <input
              ...
              className="w-full pl-9 pr-7 py-1.5 rounded-full bg-transparent text-xs font-semibold text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none"
            />
            {searchQuery && (
              <button
                type="button"
                onClick={() => setSearchQuery("")}
                className="absolute right-2.5 p-1 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 text-xs cursor-pointer"
              >
                ✕
              </button>
            )}
          ...
          {searchQuery.trim().length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 p-2 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-xl z-30 space-y-1">
              <div className="flex items-center justify-between px-2.5 py-1 text-[11px] font-bold text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800">
                <span>Luoghi salvati ({filteredPlaces.length})</span>
                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-normal">Cerca tra i tuoi spot</span>
              </div>

              {filteredPlaces.length > 0 ? (
                <div className="max-h-48 overflow-y-auto divide-y divide-slate-50 dark:divide-slate-800/60">
                  {filteredPlaces.slice(0, 5).map((p) => (
                    <button
                      ...
                      className="w-full text-left p-2 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl flex items-center justify-between gap-2 transition-colors group cursor-pointer"
                    >
              ...
              <div className="pt-1 border-t border-slate-100 dark:border-slate-800">
                <button
                  ...
                  className="w-full p-2 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-950 font-semibold text-xs flex items-center justify-center gap-1.5 transition-colors shadow-xs cursor-pointer"
                >
      ...
      <SearchFilterOverlay
        ...
        onSelectTransport={setActiveTransport}
