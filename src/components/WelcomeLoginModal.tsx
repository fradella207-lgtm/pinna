  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (msg.includes("annullato")) {
        ...
        setError("Il tuo browser ha bloccato il popup di Google. Usa l'accesso diretto inserendo la tua email nel campo sopra: è immediato e senza popup!");
      }
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        <div className="p-5 sm:p-6 space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-2.5 px-4 rounded-xl bg-white border border-slate-300 hover:border-slate-400 hover:bg-slate-50 text-slate-800 font-bold text-xs shadow-xs flex items-center justify-center gap-2.5 transition-all active:scale-[0.98] disabled:opacity-60"
          ...
          <div className="pt-2 flex items-center justify-center gap-1.5 text-[11px] text-slate-400 text-center">
            ...
            <span>I tuoi spot sono salvati in sicurezza sul tuo account personale</span>
          </div>
        </div>
      </motion.div>
    </motion.div>
