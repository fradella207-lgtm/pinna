...
export const AuthModal: React.FC = () => {
  ...
  // Ultra-simple direct sign in: no popups, zero friction, works everywhere
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (msg.includes("annullato")) {
        setError("Accesso con Google annullato.");
      } else {
        setError("Il browser ha bloccato il popup di Google. Nessun problema: inserisci la tua email nel campo qui sopra ed entra subito senza popup!");
      }
    } finally {
      setLoading(false);
    }
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Inserisci la tua email per sincronizzare i tuoi spot
              </p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          <form onSubmit={handleDirectSignIn} className="space-y-3">
            ...
            <div className="flex items-center justify-between pt-0.5">
              ...
              >
                ...
                <span>{withPassword ? "Rimuovi password (accesso diretto)" : "Vuoi inserire una password?"}</span>
              </button>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs sm:text-sm shadow-md flex items-center justify-center gap-2 transition-all active:scale-98 disabled:opacity-60 cursor-pointer"
            >
              {loading ? (
          ...
          <button
            ...
            title="Accedi con account Google"
          >
          ...
          <div className="pt-2 text-center">
            <button
              ...
              onClick={handleGuest}
              disabled={loading}
              className="text-xs text-slate-500 hover:text-slate-900 font-semibold inline-flex items-center gap-1.5 transition-colors"
            >
          ...
          <div className="pt-2 flex items-center justify-center gap-1.5 text-[11px] text-slate-400 text-center">
            ...
            <span>Dati salvati sul tuo account personale</span>
          </div>
        </div>
      </motion.div>
    </motion.div>
